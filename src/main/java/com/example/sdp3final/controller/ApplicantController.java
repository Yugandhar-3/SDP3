...
import com.example.sdp3final.repository.FoodTypeRepository;
...
public class ApplicantController {
    ...
    @RequestMapping("calculatediet")
    public String showDietPage(HttpServletRequest request){
        request.setAttribute("foodtypes", adminService.showAllFoodTypes());
        request.setAttribute("mode","VIEW_FOODFORM");
        return "calculatenutrition";
    }

    @RequestMapping("addList/{fname}")
    public String showNutriotionForm(@PathVariable("fname") String fname, HttpServletRequest request){
        String username=request.getSession().getAttribute("username1").toString();
        request.setAttribute("uname",username);
        request.setAttribute("fname",fname);
        request.setAttribute("mode","SHOW_FORM");
        return "calculatenutrition";
    }

    @PostMapping("/addToList")
    public String addToList(@RequestParam(name = "uname") String uname,@RequestParam(name = "fname")
            String fname,@RequestParam(name = "quantity") String quantity,HttpServletRequest request){
            Optional<FoodType> food = userService.getFoodByName(fname);
            FoodType f= food.get();
            int carb = Integer.parseInt(f.getFcarbo());
            int prot = Integer.parseInt(f.getFprotein());
            int n = Integer.parseInt(quantity);
            WishList w = new WishList();
            w.setUname(uname);
            w.setFname(fname);
            w.setQuantity(quantity);
            w.setTotCarb(String.valueOf(n*carb));
            w.setTotProt(String.valueOf(n*prot));
            userService.saveWishList(w);
        return "redirect:/calculatediet";
    }
    ...
    public String showWishlist(HttpServletRequest request){
        ...
        Object s=request.getSession().getAttribute("username1");
        if(s!=null) {
            List<WishList> w = userService.showWishlist();
            int totCarb = calcTotCarb(w);
            int totProt = calcTotProt(w);
            request.setAttribute("wishlist", w);
            request.setAttribute("carb",totCarb);
            request.setAttribute("prot",totProt);
            request.setAttribute("mode", "ALL_WISHES");
            return "viewallwishes";
        }
        else
            return "customerhome";
    }

    private int calcTotProt(List<WishList> w) {
        int prot = 0;
        for(WishList wish:w){
            prot+=Integer.parseInt(wish.getTotProt());
        }
        return prot;
    }

    private int calcTotCarb(List<WishList> w) {
        int carb = 0;
        for(WishList wish:w){
            carb+=Integer.parseInt(wish.getTotCarb());
        }
        return carb;
    }
